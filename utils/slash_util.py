...
def describe(**kwargs: str) -> Callable[[SlashCommand], SlashCommand]:
    """
    ...
    def _inner(cmd: SlashCommand) -> SlashCommand:
        for name, desc in kwargs.values():
            if name not in cmd.parameters:
                raise ValueError(f"`{name}` is not a defined parameter")

            cmd._parameter_descriptions[name] = desc
        return cmd
    return _inner
...
class ApplicationCog(commands.Cog, Generic[BotT]):
    ...
    async def _internal_interaction_handler(self, interaction: discord.Interaction):
        ...
        resolved = interaction.data.get('resolved') # type: ignore
        if resolved:
            resolved_data = self._get_resolved_data(interaction, resolved, state)
            params = []
